...
const RESOLVER_VERSION='2026.09.23.25';
...
async function botDoAfiliadoProduct(url){
  ...
  const debug={configured:Boolean(key),url:Boolean(url),status:null,error:null};
  console.log('[botdoafiliado] configured=',debug.configured,'url=',debug.url);
  if(!key||!url)return {__debug:debug};
  try{
    ...
    if(!r.ok){debug.error=`HTTP ${r.status}`;return {__debug:debug};}
  ...
  }catch(e){debug.error=String(e?.message||e);return {__debug:debug}}
}
