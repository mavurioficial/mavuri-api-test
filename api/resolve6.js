...
const RESOLVER_VERSION='2026.09.23.27';
...
function catalogId(v){const s=String(v||'');return s.match(/\/up\/(MLBU[A-Z0-9_-]*\d+)/i)?.[1]?.toUpperCase()||s.match(/\b(MLBU[A-Z0-9_-]*\d+)\b/i)?.[1]?.toUpperCase()||''}
...
function allowedHost(value){try{const hostname=new URL(value).hostname.toLowerCase();return hostname==='meli.la'||hostname==='mercadolivre.com.br'||hostname.endsWith('.mercadolivre.com.br')||hostname==='mercadolibre.com'||hostname.endsWith('.mercadolibre.com')}catch{return false}}
async function get(url,headers={}){try{const r=await fetch(url,{redirect:'follow',cache:'no-store',headers});if(!allowedHost(r.url||url))return null;return r}catch{return null}}
...
function embeddedSocialProductData(html,query){
  const raw=String(html||'');
  const priceMatch=raw.match(/\\*"current_price\\*"\s*:\s*\\*\{\\*"value\\*"\s*:\s*([0-9]+(?:\.[0-9]+)?)/i);
  if(!priceMatch)return null;
  const previousMatch=raw.match(/\\*"previous_price\\*"\s*:\s*\\*\{\\*"value\\*"\s*:\s*([0-9]+(?:\.[0-9]+)?)/i);
  const currencyMatch=raw.match(/\\*"current_price\\*"[\s\S]{0,240}?\\*"currency\\*"\s*:\s*\\*"([A-Z]{3})/i);
  const q=String(query||'').trim();
  const price=number(priceMatch[1]);
  if(price===null||price<=0)return null;
  return {title:q,permalink:'',price,previousPrice:number(previousMatch?.[1]),image:'',currency:first(currencyMatch?.[1],'BRL'),score:1,_hasId:0,source:'affiliate-social-embedded'};
}
function socialProductData(html,query,itemId=''){
  const candidates=[];
  const add=(x)=>{
    if(!x||typeof x!=='object')return;
    const title=first(x.title,x.name,x.productName,x.itemName,x.product?.title,x.item?.title);
    const permalink=first(x.permalink,x.url,x.link,x.product?.permalink,x.item?.permalink);
    const price=number(x.price,x.current_price,x.currentPrice,x.sale_price,x.salePrice,x.product?.price,x.item?.price,x.offers?.price,Array.isArray(x.offers)?x.offers[0]?.price:null,x.price?.amount);
    const original=number(x.original_price,x.originalPrice,x.list_price,x.regular_price,x.product?.original_price,x.item?.original_price,x.offers?.highPrice);
    const image=first(x.thumbnail,x.secure_thumbnail,x.image,x.picture,x.product?.thumbnail,x.item?.thumbnail);
    if(title&&(price!==null||permalink||image))candidates.push({title,permalink,price,previousPrice:original,image,score:score(query,title),_hasId:itemId&&JSON.stringify(x).toUpperCase().includes(String(itemId).toUpperCase())?1:0});
  };
  const embedded=embeddedSocialProductData(html,query);
  ...
  if(q){let p=lower.indexOf(q);while(p>=0&&anchors.length<12){anchors.push(p);p=lower.indexOf(q,p+1)}}
  for(const p of anchors){
    const start=Math.max(0,p-1800), end=Math.min(plain.length,p+5000), window=plain.slice(start,end);
    const prices=[...window.matchAll(/R\$\s*([0-9]{1,3}(?:\.[0-9]{3})*,[0-9]{2})/g)].map(m=>number(m[1])).filter(v=>v!==null&&v>0&&v<1000000);
    if(!prices.length)continue;
    const title=q||'';
    candidates.push({title,permalink:'',price:prices[0],previousPrice:prices.length>1?prices[1]:null,image:'',score:score(query,title),_hasId:iid&&window.toUpperCase().includes(iid)?1:0});
  }
  candidates.sort((a,b)=>b._hasId-a._hasId||b.score-a.score);
  return candidates.find(x=>x.price!==null&&x.price>0&&(!q||x.score>=0.5))||null;
}
async function botDoAfiliadoProduct(url){
  ...
  try{
    ...
    const item=Array.isArray(d?.items)?d.items[0]:d?.item||d?.data?.items?.[0]||d?.data||d;
    if(!item||typeof item!=='object')return null;
    const title=first(item.title,item.name,item.product?.title);
    const price=number(item.price,item.current_price,item.currentPrice,item.sale_price,item.product?.price,item.data?.price);
    const previousPrice=number(item.old_price,item.previous_price,item.original_price,item.list_price,item.product?.old_price,item.product?.original_price);
    const image=first(item.image_url,item.image,item.thumbnail,item.product?.image_url,item.product?.thumbnail);
    const permalink=first(item.permalink,item.url,item.final_url,item.product?.permalink,item.product?.url);
    if(!title&&!price&&!image)return null;
    return {title,price,previousPrice,image,permalink,source:'botdoafiliado'};
  }catch{return null}
}
function meaningfulQuery(v){
  const s=String(v||'').replace(/\s+/g,' ').trim();
  if(!s)return '';
  if(/^(?:p|up|mlb\d+|mlbu[a-z0-9_-]*\d+)$/i.test(s))return '';
...
async function enrich(product,id,query,authorization,catalogProductId=''){
  ...
  const catalog=await json(`https://api.mercadolibre.com/products/${encodeURIComponent(catalogProductId||id)}`);
  if(catalog){
