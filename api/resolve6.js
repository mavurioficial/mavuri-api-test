...
const RESOLVER_VERSION='2026.09.23.17';
...
async function webSearchProduct(query,itemId=''){
  if(!query)return null;
  try{
    const q='"'+String(query).trim()+'" site:mercadolivre.com.br';
    const r=await get('https://www.bing.com/search?q='+encodeURIComponent(q)+'&count=10',{'user-agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/126 Safari/537.36','accept-language':'pt-BR,pt;q=0.9','accept':'text/html,application/xhtml+xml'});
    if(!r?.ok)return null;
    const html=await r.text();
    const blocks=[...html.matchAll(/<li[^>]+class=["']b_algo["'][^>]*>[\s\S]*?<\/li>/gi)].map(m=>m[0]);
    const pool=blocks.length?blocks:[html];
    for(const block of pool){
      const text=block.replace(/<script[\s\S]*?<\/script>/gi,' ').replace(/<style[\s\S]*?<\/style>/gi,' ').replace(/<[^>]+>/g,' ').replace(/&nbsp;/gi,' ').replace(/&quot;/gi,'"').replace(/&amp;/gi,'&').replace(/\s+/g,' ').trim();
      const titleMatch=block.match(/<h2[^>]*>[\s\S]*?<a[^>]*>([\s\S]*?)<\/a>[\s\S]*?<\/h2>/i);
      const title=String(titleMatch?.[1]||'').replace(/<[^>]+>/g,' ').replace(/&amp;/g,'&').trim();
    ...
    const plain=html.replace(/<script[\s\S]*?<\/script>/gi,' ').replace(/<style[\s\S]*?<\/style>/gi,' ').replace(/<[^>]+>/g,' ').replace(/&nbsp;/gi,' ').replace(/&quot;/gi,'"').replace(/&amp;/gi,'&').replace(/\s+/g,' ').trim();
    const pos=plain.toLowerCase().indexOf(String(query).toLowerCase());
    if(pos>=0){
      const window=plain.slice(Math.max(0,pos-1200),Math.min(plain.length,pos+5000));
      const prices=[...window.matchAll(/R\$\s*([0-9]{1,3}(?:\.[0-9]{3})*,[0-9]{2})/gi)].map(m=>number(m[1])).filter(v=>v!==null&&v>0&&v<1000000);
      if(prices.length)return {title:query,price:prices[0],previousPrice:prices.length>1?prices[1]:null,permalink:''};
    }
    return null;
  }catch{return null}
}
